...
PyObject * CudaNdarray_As_Buffer(CudaNdarray * self)
{
    int size=0;
    int offset=0;
	
	size = CudaNdarray_SIZE(self)*4;

    PyObject * buffer_return = (PyObject *)(PyBuffer_FromReadWriteMemory((void *) (self->devdata + offset), size));

    Py_INCREF(buffer_return);

    return buffer_return;
}

PyObject * CudaNdarray_View_Buffer(CudaNdarray * self)
{
    ...
    Py_buffer * buffer_return = (Py_buffer *)(PyBuffer_FromReadWriteMemory((void *) (self->devdata + offset), Py_END_OF_BUFFER));
	
	PyObject * mem_view = PyMemoryView_FromBuffer(buffer_return);
		
	Py_INCREF(mem_view);
	Py_DECREF(buffer_return);
	
	return mem_view;
	
}

PyObject * CudaNdarray_View_Info(CudaNdarray * self)
{
    int size=0;
	
    int offset=0;
	
	size = CudaNdarray_SIZE(self); // nbytes = size*4
	
	printf("size:%i \n", size);
	
	for(int _i=0;_i< (self->nd); _i++)
		printf("dim %i :%i \n", _i,  CudaNdarray_HOST_DIMS(self)[_i]);
		

    //PyObject * buffer_return = (PyObject *)(PyBuffer_FromReadWriteMemory((void *) (arr->devdata + offset), size));
...
PyObject * CudaNdarray_Release_Buffer(PyObject *self, PyObject *args)
{
...
static PyMethodDef CudaNdarray_methods[] =
{
    ...
    {"as_buffer",
        (PyCFunction)CudaNdarray_As_Buffer, METH_NOARGS,
        "Get a buffer pointer from the CudaNdarray to be used in GPU memory transfer"}, 
	{"view_buffer",
	        (PyCFunction)CudaNdarray_View_Buffer, METH_NOARGS,
	        "view buffer"}, 
			
	{"view_info",
		(PyCFunction)CudaNdarray_View_Info, METH_NOARGS,
		"view info"	},
    {"release_buffer",
        (PyCFunction)CudaNdarray_Release_Buffer, METH_VARARGS,
        "Release a buffer pointer from the device"},
    {"__copy__",
