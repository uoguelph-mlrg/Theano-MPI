'''
This test case is for testing the as_buffer function
...
if __name__ == '__main__':
    ...
    if rank==0:
        ...
        print 'ori', arr.get_value()[0][0:3]
        
        buf_in = arr.container.data.as_buffer()
        
    
        comm.Send([buf_in, MPI.FLOAT], dest=1)
        
        
    else:
        
        arr.set_value(numpy.zeros(arr.get_value().shape, dtype=numpy.float32))
        
        buf_out = arr.container.data.as_buffer()
        
        comm.Recv([buf_out, MPI.FLOAT], source=0)
        
        print 'recv' , arr.get_value()[0][0:3]
